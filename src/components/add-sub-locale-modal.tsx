...
import { LOCALE_NAMES, localeName } from "@/lib/asc/locales";
...
interface Props {
  ...
  /** Used as fallback `name` if no source localization name is provided. ASC rejects empty names. */
  defaultName?: string;
  /** Optional: copy these as initial name/description for the new localization. */
  sourceName?: string;
...
export function AddSubLocaleModal({ open, onClose, accountId, subId, existingLocales, defaultName, sourceName, sourceDescription }: Props) {
  const t = useT();
  ...
  const add = useMutation({
    mutationFn: async () => {
      ...
      // ASC requires `name` to be a non-empty string. We seed it from the source
      // localization's name, fall back to `defaultName` (typically the
      // subscription's reference name), and as a last resort use the locale code.
      const nameSeed = (sourceName?.trim() || defaultName?.trim() || "").slice(0, 30);
      const descSeed = (sourceDescription?.trim() || "").slice(0, 45);
        ...
        locales.map(async (locale) => {
          try {
            const attrs: Record<string, string> = {
              locale,
              name: nameSeed || locale,
            };
            if (descSeed) attrs.description = descSeed;
      ...
      qc.invalidateQueries({ queryKey: ["asc", accountId, "subscription", subId, "localizations"] });
      setPhase("done");
    },
  });
          ...
          <>
            <div className="p-5 border-b border-[var(--border)] space-y-2">
              ...
              <div className="flex items-center justify-between gap-3 flex-wrap">
                <p className="text-xs text-[var(--muted-foreground)]">
                  {t.addLocale.selected}: {selected.size} · {t.addLocale.available}: {candidates.length}
                </p>
                <div className="flex items-center gap-2">
                  <button
                    type="button"
            ...
            <div className="flex-1 overflow-auto p-3 grid sm:grid-cols-2 gap-1.5">
              {candidates.map((loc) => {
                ...
                return (
                  ...
                  >
                    <input type="checkbox" checked={checked} onChange={() => toggle(loc)} className="accent-[var(--foreground)]" />
                    <span className="font-mono text-xs">{loc}</span>
                    <span className="truncate text-xs text-[var(--muted-foreground)]">{localeName(loc)}</span>
                  </label>
                );
              })}
              {candidates.length === 0 && (
            ...
            <div className="flex items-center justify-end gap-2 p-5 border-t border-[var(--border)]">
              ...
              <Button onClick={() => add.mutate()} disabled={selected.size === 0}>
                ...
                {selected.size > 0 ? interpolate(t.addLocale.addN, { n: selected.size }) : t.addLocale.addNone}
              </Button>
            </div>
          </>
        )}
            ...
            <div className="flex-1 overflow-auto p-5 space-y-1.5">
              {results.map((r) => (
                ...
                >
                  ...
                  <span className="text-xs text-[var(--muted-foreground)] flex-1 truncate">{localeName(r.locale)}</span>
